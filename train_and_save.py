...
print("--- HI this is parthiv , starting the model ---")
...
def process_file(file_path, is_mc=False):
    ...
    for i in range(4):
        ...
        df[f'lep_pt_{i}'] = leptons.pt[:, i] / 1000 #<- here 
        df[f'lep_eta_{i}'] = leptons.eta[:, i]
        df[f'lep_phi_{i}'] = leptons.phi[:, i]
        df[f'lep_E_{i}'] = leptons.E[:, i] / 1000 #<- here
...
weights[weights < 0] = 0 # Handle negative weights {IMP DONT REMOVE OR DEFY}

X_train, X_test, y_train, y_test, w_train, w_test = train_test_split(
    X, y, weights, test_size=0.3, random_state=42, stratify=y)


print("\nModel initiated -> xgboost ")
